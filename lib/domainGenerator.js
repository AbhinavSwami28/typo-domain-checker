...
// Common homoglyph substitutions (including internationalized and multi-char)
const HOMOGLYPHS = {
  a: ["4", "@", "à", "á", "â", "ã", "ä"],
  b: ["d", "6", "lb"],
  c: ["k", "ç", "e"],
...
// Consonant pairs that are commonly confused (sound-alike or look-alike)
...
// Unicode confusables — Greek and Armenian lookalikes (beyond Cyrillic)
const GREEK_CONFUSABLES = {
  a: "α", b: "β", e: "ε", h: "η", i: "ι",
  k: "κ", n: "ν", o: "ο", p: "ρ", t: "τ",
  u: "υ", x: "χ", y: "γ",
};
const ARMENIAN_CONFUSABLES = {
  a: "ɑ", g: "ց", h: "հ", n: "ո", o: "օ",
  p: "р", q: "զ", s: "ʂ", u: "ս",
};

// Combo squatting — phishing/impersonation keywords attackers combine with brands
const COMBO_WORDS = [
  "login", "signin", "verify", "confirm", "update", "secure",
  "account", "payment", "billing", "renew", "alert", "notice",
  "support", "help", "service", "official", "auth", "validate",
  "password", "reset", "recovery", "access", "portal", "admin",
];
...
export function generateTypoDomains(domain) {
  ...
  // 30b. Dash to dot — replace hyphens with dots (creates subdomain variants)
  //      e.g., my-example.com → my.example.com (attacker registers example.com)
  if (name.includes("-")) {
    const hyphenPositions = [];
    for (let i = 0; i < name.length; i++) {
      if (name[i] === "-") hyphenPositions.push(i);
    }
    // Replace each hyphen with a dot one at a time
    for (const pos of hyphenPositions) {
      const dotted = name.slice(0, pos) + "." + name.slice(pos + 1);
      add(dotted + "." + tld, "Dash to Dot");
    }
    // Replace ALL hyphens with dots
    if (hyphenPositions.length > 1) {
      add(name.replace(/-/g, ".") + "." + tld, "Dash to Dot");
    }
  }
  ...
  // 35. Consonant swap — replace consonants with commonly confused counterparts
  //     e.g., netflix → netflik (x↔k), credit → gredit (c↔g)
  for (let i = 0; i < name.length; i++) {
    const swaps = CONSONANT_SWAPS[name[i].toLowerCase()];
    if (swaps) {
      for (const s of swaps) {
        add(name.slice(0, i) + s + name.slice(i + 1) + "." + tld, "Consonant Swap");
      }
    }
  }

  // 36. Unicode confusables — Greek and Armenian lookalike characters
  //     Extends Cyrillic IDN (#22) with additional scripts
  for (let i = 0; i < name.length; i++) {
    const ch = name[i].toLowerCase();
    const greek = GREEK_CONFUSABLES[ch];
    if (greek) {
      add(name.slice(0, i) + greek + name.slice(i + 1) + "." + tld, "Greek Confusable");
    }
    const armenian = ARMENIAN_CONFUSABLES[ch];
    if (armenian) {
      add(name.slice(0, i) + armenian + name.slice(i + 1) + "." + tld, "Armenian Confusable");
    }
  }

  // 37. Combo squatting — combine brand with phishing/impersonation keywords
  //     e.g., netflix-login.com, loginnetflix.com, netflix-verify-account.com
  for (const word of COMBO_WORDS) {
    add(name + "-" + word + "." + tld, "Combo Squatting");
    add(word + "-" + name + "." + tld, "Combo Squatting");
    add(name + word + "." + tld, "Combo Squatting");
    add(word + name + "." + tld, "Combo Squatting");
  }
