...
// ---------------------------------------------------------------------------
// LRU Cache — keeps recent lookups in memory (per serverless invocation or
// server lifetime). Avoids hammering external services for the same domain.
// ---------------------------------------------------------------------------
class LRUCache {
  constructor(max = 500) {
    this.max = max;
...
// ---------------------------------------------------------------------------
// Main entry: checkDomain — runs the multi-source lookup strategy
//
// Strategy:
//   Phase 1 (parallel, fast): DNS checks + HTTP probe + RDAP
//     - DNS (native + Google DoH + Cloudflare DoH) confirms existence
//     - HTTP probe confirms active web presence
//     - RDAP provides registration details (registrar, dates)
//   Phase 2 (sequential fallbacks): RDAP proxy → WHOIS
//     - Only runs if Phase 1 RDAP failed to get details
//
// Deduplication: DNS/HTTP confirm registration; RDAP/WHOIS provide details.
// We merge: if DNS confirms registered but RDAP failed, we still report
// registered and try WHOIS for details.
// ---------------------------------------------------------------------------
export async function checkDomain(domain) {
  ...
  // Phase 1: Run DNS, HTTP probe, and RDAP in parallel
  ...
  // If RDAP gave us a definitive answer with details, use it
  ...
  if (rdapResult && rdapResult.registered === false) {
    // RDAP says not registered — but check if DNS/HTTP disagree
    if (dnsResult !== "registered" && httpResult !== "registered") {
      cache.set(domain, rdapResult);
      return rdapResult;
    }
    // DNS or HTTP says registered despite RDAP 404 — domain may be newly registered
    // or RDAP data is stale. Trust DNS/HTTP and try to get details from other sources.
  }

  // Phase 2: RDAP didn't give details — try fallbacks
  // But first check: do DNS/HTTP confirm registration?
  ...
  // Check if DNS conclusively said not registered
  if (dnsResult === "not_registered" || httpResult === "not_registered") {
...
// ---------------------------------------------------------------------------
export async function checkBatch(domains, maxConcurrency = 10) {
  let idx = 0;
